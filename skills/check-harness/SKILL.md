...
Agent(
  ...
  prompt: """
    ...
    추가 수집 대상: enabledPlugins (user/project), mcpServers (user + project .mcp.json),
    installed plugins 인벤토리, MCP 호출 사용도(최근 30일).
    SKILL.md 정의된 JSON 스키마 + current_state/unused_mcp/plugin_findings 필드 포함.
...
- **Runtime 인벤토리(PORTFOLIO.current_state / unused_mcp / plugin_findings) 기반 제안 최소 1개 포함**:
  - 사용 0회 MCP → "이 프로젝트에선 disable 고려" 액션
  - `installed_not_enabled` 플러그인 중 최근 세션에서 필요했을 수 있는 것 → "활성화 검토" 액션
  - `enabled_unused` 플러그인 → "비활성화해서 컨텍스트 절약" 액션
...
```
👤 User Scope    (유저 전역)    L{n}  ▓▓▓▓▓▓▓░░░  {XX}% → L{n+1}   (Score: {NN})
📁 Project Scope ({project})    L{n}  ▓▓▓▓▓▓▓▓░░  {XX}% → L{n+1}   (Score: {NN})
```

> **User Scope**: `~/.claude/` 전체에 해당 — 설치된 스킬 정리 상태(A축) + 평소 세션 실행 습관(B축).
> **Project Scope**: 현재 프로젝트 한정 — CLAUDE.md/경계(C축) + 프로젝트 자동화·검증(D축).
> 진행바는 다음 레벨까지의 진척도이고, 점수는 다음 진단 때와 비교하세요.
...
> 부담 없이 골라서 하세요. 난이도/임팩트 순이며, 각 액션에 **적용 범위(👤/📁)** 를 표시했습니다.
> 👤 User Scope = `~/.claude/` 유저 전역에 반영되는 변경 · 📁 Project Scope = 이 프로젝트 폴더 한정

### 🟢 바로 해볼 만한 것
- [ ] {👤 User | 📁 Project} **{action}** — 예상 효과: {benefit}
...
| Scope | Axis                          | Score | Level | 다음 레벨까지 |
|-------|-------------------------------|------:|:-----:|:-------------|
| 👤 User    | A. Portfolio Hygiene (스킬 정리)          |  {NN} | L{n}  | {XX}% → L{n+1} |
| 👤 User    | B. Execution Patterns (세션 실행 습관)      |  {NN} | L{n}  | {XX}% → L{n+1} |
| 📁 Project | C. Context & Boundaries (맥락·경계)       |  {NN} | L{n}  | {XX}% → L{n+1} |
| 📁 Project | D. Automation & Verification (자동화·검증) |  {NN} | L{n}  | {XX}% → L{n+1} |
...
> 지금 이 Claude Code 세션에서 **실제로 접근 가능한** 것들 — 설치만 된 것이 아니라 enabled 상태.
...
> 💡 "👤" = user level(`~/.claude/`), "📁" = 이 프로젝트(`{PROJECT_ROOT}/.claude/`)에서 활성
...
<details>
<summary>🧩 Skill Portfolio 상세 (👤 User Scope)</summary>

전체 설치 스킬: {N}개 · 최근 30일 사용: {X}개 ({%})

| 분류 | 개수 | 설명 | 예시 |
|------|----:|------|------|
| 😴 오래 안 쓴 스킬 (90일+ 미사용) | N | 설치는 되어있는데 실제로 호출이 오래 없는 것 | ... |
| 👻 기록만 남은 스킬 | N | 사용 기록에는 있는데 실제 설치는 없는 유령 엔트리 (이전 plugin 제거 흔적) | ... |
| 🔁 비슷한 기능이 여러 개 | N | 같은 목적의 스킬이 이름만 다르게 여러 개 존재 (예: code-review 계열) | ... |
| 🏷️ 네임스페이스 중복 | N | `foo` 와 `bar:foo` 처럼 루트/플러그인에 같은 이름이 공존 — AI가 어느 쪽을 쓸지 혼동 | ... |
| ⚠️ 트리거 키워드 겹침 | N | 여러 스킬이 같은 트리거 문구를 주장 — 발동이 예측 불가해짐 | ... |

**정리 제안**
- 제거 후보: {skill} (최근 사용 0회, 설치 45일 경과)
- 통합 제안: "code review" 류 스킬 여러 개 → `{survivor}` 하나로 모으기
- 이름 겹침: `dev-scan` vs `dev:dev-scan` — 하나만 남기기

> 💡 용어 안내: "오래 안 쓴 스킬"을 정리하면 AI가 트리거할 후보가 줄어 정확도가 올라갑니다. "기록만 남은 스킬"은 과거 설치했다 제거한 흔적이라 무해하지만 분석 노이즈를 만듭니다.

</details>

<details>
<summary>⚡ Execution Pattern 상세 (👤 User Scope · 최근 세션 실행 습관)</summary>

- 계획 먼저 짜는 비율 (plan-first):     {X}%
- 서브에이전트 위임 비율 (delegation): {X}%
- 병렬 호출 횟수 (parallel):           {N}
- 세션 종료 시 인수인계 비율 (handoff): {X}%
- 완료 후 검증 비율 (completion check): {X}%
- 반복 동작 패턴 비중 (top 3-gram):     {X}%  {⚠️ if > 30% — 자동화 후보}
